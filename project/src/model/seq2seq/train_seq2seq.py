...
from model.seq2seq.seq2seq_data_util import generate_meo_forecast, get_training_statistics, generate_dev_set, generate_aggr_set, \
    generate_X_test_set
...
def train_model(city='bj',
    ...
    gap : 0,12,24
        0 : 当天 23点以后进行的模型训练， 最终选择0， 因为最后进行预测的时候给了前天23点的数据，正好衔接
        12 : 当天中午进行的模型训练
        24 : 不使用当天数据进行的训练
    ...
    if generate_mean and generate_range:
        ...
        input_dim = len(station_list) * (len(X_aq_list) + len(X_meo_list))  # 35*(6+5)=385
        output_dim = len(station_list) * len(y_aq_list)  # 35*3=105
    ...
    with tf.Session() as sess:
        ...
        for i in range(total_iteractions):
            ...
            if use_norm_data and loss_weights:
                _, _, batch_output_original = SMAPE_on_dataset_v1(batch_output, batch_output, output_features,
                                                                  statistics, 1)
                batch_output_original += 1e-3  # for numerical stability

            print('batch_input.shape and batch_output.shape:', batch_input.shape, batch_output.shape)

            feed_dict = {rnn_model['enc_inp'][t]: batch_input[:, t, :] for t in range(input_seq_len)}
            feed_dict.update({rnn_model['target_seq'][t]: batch_output[:, t, :] for t in range(output_seq_len)})

            if use_norm_data and loss_weights:
                feed_dict.update({rnn_model['target_seq_original'][t]: batch_output_original[:, t, :] for t in
            ...
            if i % 5 == 0:
                ...
                if not os.path.isdir(path_to_city):
                    os.makedirs(path_to_city)
    ...
    if predict_one_day:
        ...
        for name in saved_iteractions:
            ...
            with tf.Session() as sess:
                sess.run(init)
                saver = rnn_model['saver']().restore(sess, os.path.join(path_to_city, name))
                feed_dict = {rnn_model['enc_inp'][t]: dev_x_shift[:, t, :] for t in
                             range(input_seq_len)}  # batch prediction
                feed_dict.update(
                    {rnn_model['target_seq'][t]: np.zeros([dev_x_shift.shape[0], output_dim], dtype=np.float32) for t in
                     range(output_seq_len)})
            ...
            if use_norm_data:
                aver_smapes, smapes_of_features, model_preds_on_dev = SMAPE_on_dataset_v1(dev_y, final_preds,
        ...
        with tf.Session() as sess:
            ...
            feed_dict.update(
                {rnn_model['target_seq'][t]: np.zeros([aggr_x.shape[0], output_dim], dtype=np.float32) for t in
                 range(output_seq_len)})
            y_day_one = sess.run(rnn_model['reshaped_outputs'], feed_dict)
        ...
        with tf.Session() as sess:
            sess.run(init)
            saver = rnn_model['saver']().restore(sess, os.path.join(path_to_city, name))
            feed_dict = {rnn_model['enc_inp'][t]: aggr_x_shift[:, t, :] for t in
                         range(input_seq_len)}  # batch prediction
            feed_dict.update(
                {rnn_model['target_seq'][t]: np.zeros([aggr_x_shift.shape[0], output_dim], dtype=np.float32) for t in
                 range(output_seq_len)})
        ...
        if use_norm_data:
            _, _, model_preds_on_aggr = SMAPE_on_dataset_v1(final_aggr_preds, final_aggr_preds, output_features,
                                                            statistics, 1)  # 仅仅是为了计算预测值
        else:
        ...
        with tf.Session() as sess:
            sess.run(init)
            saver = rnn_model['saver']().restore(sess, os.path.join(path_to_city, name))
            feed_dict = {rnn_model['enc_inp'][t]: test_x_shift[:, t, :] for t in
                         range(input_seq_len)}  # batch prediction
            feed_dict.update(
                {rnn_model['target_seq'][t]: np.zeros([test_x_shift.shape[0], output_dim], dtype=np.float32) for t in
                 range(output_seq_len)})
        ...
        if use_norm_data:
            _, _, model_preds_on_test = SMAPE_on_dataset_v1(final_test_preds, final_test_preds, output_features,
                                                            statistics, 1)  # 仅仅是为了计算预测值
        else:
    ...
    # 3.2 predict_one_day = False
        ...
        model_name = saved_iteractions[0]
        for name in saved_iteractions:
            ...
            with tf.Session() as sess:
                ...
                if use_day_model:
                    if generate_mean and generate_range:
                        ...
                        range_preds = final_preds[:, :,
                                      int(output_dim / 2):]  # (m, output_seq_len, output_dim/2) predictions of ranges
